...
##########################
# File: ESPNWeekly.R
# Description: Scrapes Weekly Projections from ESPN
# Date: 10/19/2015
...
espn_base_url <- paste0("http://games.espn.go.com/ffl/tools/projections?")
espn_pos <- list(QB=0, RB=2, WR=4, TE=6, K=17, DST=16)
espn_pages <- c("0","40","80")
espn_urls <- paste0(espn_base_url, "&slotCategoryId=", rep(espn_pos, each=length(espn_pages)), "&startIndex=", espn_pages,"&scoringPeriodId=",weekNo,"&seasonId=",season)
...
#Clean data
qbNames <- rbNames <- wrNames <- teNames <- kNames <- dstNames <- c("player","opp","status","passCompAtt","passYds","passTds","passInt","rushAtt","rushYds","rushTds","rec","recYds","recTds","points")
...
save(projections_espn, file = paste(getwd(), "/Data/Historical Projections/ESPN-Projections-", season,"-week-",weekNo, ".RData", sep=""))
write.csv(projections_espn, file=paste(getwd(), "/Data/Historical Projections/ESPN-Projections-", season,"-week-",weekNo, ".csv", sep=""), row.names=FALSE)
