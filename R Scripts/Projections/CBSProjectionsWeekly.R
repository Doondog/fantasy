...
#cbs_writers <- c("jamey_eisenberg","dave_richard") #Commented out as it appears that the url has changed
#cbs_source <- c("cbs1", "cbs2") #commented out as it appears that the url has changed
cbs_leaguetype <- "standard"
cbs_urls <- paste0(cbs_baseurl, cbs_pos, "/", weekNo, "/avg/", cbs_leaguetype,"?&print_rows=9999")

#Scrape 
cbs <- lapply(cbs_urls, function(x) {data.table(readHTMLTable(x, stringsAsFactors = FALSE)[[1]])}) #The output of the readHTML table is actually a list, hence subsetting the [[1]]
cbsList <- cbs
...
for(i in 1:length(cbsList)){
  ...
  cbsList[[i]][,pos := as.factor(cbs_pos)[i]]
  ...
  if(unique(cbsList[[i]][,pos]) == "QB"){
    setnames(cbsList[[i]], c(qbNames,  "pos"))
  } else if(unique(cbsList[[i]][,pos]) == "RB"){
    setnames(cbsList[[i]], c(rbNames,  "pos"))
  } else if(unique(cbsList[[i]][,pos]) == "WR"){
    setnames(cbsList[[i]], c(wrNames,  "pos"))
  } else if(unique(cbsList[[i]][,pos]) == "TE"){
    setnames(cbsList[[i]], c(teNames,  "pos"))
  } else if(unique(cbsList[[i]][,pos]) == "K"){
    setnames(cbsList[[i]], c(kNames,  "pos"))
  } else if(unique(cbsList[[i]][,pos]) == "DST"){
    setnames(cbsList[[i]], c(dstNames,  "pos"))
  }
}
...
duplicateCases_cbs <- projections_cbs[duplicated(name)]$name
projections_cbs[which(name %in% duplicateCases_cbs),]

#Rename Players
projections_cbs[name=="TIMOTHYWRIGHT", name:="TIMWRIGHT"]

#Calculate Overall Rank
projections_cbs <- projections_cbs[order(-points)][,overallRank := 1:.N, ]

#Calculate Position Rank
projections_cbs <- projections_cbs[order(-points)][,positionRank := 1:.N, by=list(pos)]
